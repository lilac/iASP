...
#
# Converts flat clauses to incremental logic program 
...
def clauseToConstraint(cla,name):
  ...
  if len(variables) == 0:
	...
	sys.stdout.write('%s, '%(constraint[i][1]))
      ...
      sys.stdout.write('fdom(%s), 1 [ arg(%s,__inc) ].\n\n' %(args, args))


def main():
  sys.stdout.write('#cumulative __inc.\n\n')
  parseFileTPTPCNF()
  ...
  for f in input_functions:
    ...
    sys.stdout.write('dom(%s(%s)) :- fdom(%s), 1 [ arg(%s,__inc) ].\n' %(f[0],args,args2,args2))
    sys.stdout.write('functerm(%s(%s)) :- fdom(%s), 1 [ arg(%s,__inc) ].\n' %(f[0],args,args2,args2))
    sys.stdout.write('{ assign(%s(%s),Y) } :- fdom(%s;Y), 1 [ arg(%s;Y,__inc) ].\n' %(f[0],args,args2,args2))
  ...
  for f in internal_predicates:
    ...
    args = reduce(lambda x,y:x+','+y, map(lambda x: 'X'+str(x), range(0,f[1])))
    args2 = reduce(lambda x,y:x+';'+y, map(lambda x: 'X'+str(x), range(0,f[1])))
    sys.stdout.write('{ %s(%s) } :- fdom(%s), 1 [ arg(%s,__inc) ].\n' %(f[0],args,args2,args2))
  if len(tmp) != 0:
    sys.stdout.write('#base.\n')
  for f in tmp:
    sys.stdout.write('{ %s }.\n' %(f[0]))

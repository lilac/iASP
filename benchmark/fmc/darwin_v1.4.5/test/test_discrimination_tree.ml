(* $Id: test_discrimination_tree.ml,v 1.2 2004/06/13 18:57:28 alexf Exp $ *)

type term = Term.term
type subst = Subst.subst

type t_predicate_index = term Term_indexing.predicate_index
type t_index = term Term_indexing.index
type discrimination_tree = t_predicate_index
...
(*
let tree : t_predicate_index =
  Discrimination_tree.create_predicate_index (new data)
*)
(*
module TermDiscriminationTree =
  Discrimination_tree.Make (
    struct
      type t = term
...
type discrimination_tree =
    TermDiscriminationTree.t
*)
...
let validate_tree (tree: discrimination_tree) : unit =
  tree#iter
    (fun content data ->
       if not (Term.are_terms_variants content data) then begin
	 print_endline (tree#to_string);
...
let validate_tree_contains (tree: discrimination_tree) (expected: 'a list) : unit =
  List.iter
    (fun data ->
       let contained =
	 tree#find_all_variants data
       in
	 match contained with
	   | [] ->
	       print_endline ("validate_tree_contains:");
	       print_endline (tree#to_string);
	       OUnit.assert_failure
	   ...
	   | data :: _ ->
	       ()
(*	       print_endline (TermDiscriminationTree.to_string tree);
	       OUnit.assert_failure
...
let build_tree (nodes: string list) : discrimination_tree =
(*  print_endline ("build_tree");*)
  let tree =
    Discrimination_tree.create_predicate_index true (new data)
  in
  let terms =
...
let produced =
  Read_darwin.to_term "my3(a, a, a)"
in
  match TermDiscriminationTree.find_non_productive tree producer produced with
    | None ->
...
(*** insert ***)
let insert ?(success_desired:bool = true) (tree: discrimination_tree) (term: term) =
  fun () ->
...
(*** remove_variant ***)
let remove_variant ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
    ...
    in
      ...
      ignore (tree#remove term (Some term));
      
      validate_tree_contains tree expected;

      let contained =
	tree#find_all_variants term
      in
	match contained with
	  | [] ->
	      ()
...
(*** p_generalization ***)
let p_generalization ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    let nodes =
      tree#find_all_generalizations ~p_preserving:true term
    in
    let all_success =
      Tools.lists_unordered_equal Term.are_terms_variants nodes expected
    in
      OUnit.assert_bool
    ...
    let node =
      tree#find_generalization ~p_preserving:true term
    in
    let first_success =
      match node with
	| None ->
	...
	| Some term ->
	    List.exists (Term.are_terms_variants term) expected
    in
      OUnit.assert_bool
...
(*** generalization ***)
let generalization ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      tree#find_all_generalizations ~p_preserving:false term
    in
    let success =
      Tools.lists_unordered_equal Term.are_terms_variants nodes expected
    in
    ...
    let node =
      tree#find_generalization ~p_preserving:false term
    in
    let first_success =
      match node with
	| None ->
...
(*** p_unifiable ***)
let p_unifiable ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      tree#find_all_unifiable ~p_preserving:true term
    in
    let success =
      Tools.lists_unordered_equal Term.are_terms_variants nodes expected
    in
    ...
    let node =
      tree#find_unifiable ~p_preserving:true term
    in
    let first_success =
      match node with
	| None ->
...
(*** unifiable ***)
let unifiable ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      tree#find_all_unifiable ~p_preserving:false term
    in
    let success =
      Tools.lists_unordered_equal Term.are_terms_variants nodes expected
    in
    ...
    let node =
      tree#find_unifiable ~p_preserving:false term
    in
    let first_success =
      match node with
	| None ->
...
let tree =
  build_tree ["product(multiply(_0, _1), inverse(_1), _0)"; "product(multiply(_0, identity), identity, _0)"]
let test_unifiable23 =
  OUnit.TestCase (unifiable tree "product(_0, a, inverse(_1))" [])

let tree =
  build_tree ["product(multiply(_0, _1), inverse(_1), _0)"; "product(multiply(_0, identity), a, b)"]
let test_unifiable24 =
  OUnit.TestCase (unifiable tree "product(_0, a, b)" ["product(multiply(_0, identity), a, b)"])
...
(*** p_instance ***)
let p_instance ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      tree#find_all_instances ~p_preserving:true term
    in
    let success =
      Tools.lists_unordered_equal Term.are_terms_variants nodes expected
    in
    ...
    let node =
      tree#find_instance ~p_preserving:true term
    in
    let first_success =
      match node with
	| None ->
...
let tree =
  build_tree ["p(g(g(g(=U))))"]
let test_p_instance16 =
  OUnit.TestCase (p_instance tree "p(g(g(=U)))" [])

let tree =
  build_tree ["p(g(g(g(X))))"]
let test_p_instance17 =
  OUnit.TestCase (p_instance tree "p(g(g(X)))" ["p(g(g(g(X))))"])
...
(*** instance ***)
let instance ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      tree#find_all_instances ~p_preserving:false term
    in
    let success =
      Tools.lists_unordered_equal Term.are_terms_variants nodes expected
    in
    ...
    let node =
      tree#find_instance ~p_preserving:false term
    in
    let first_success =
      match node with
	| None ->
...
(*** renumber_term ***)
let renumber_term ?(success_desired:bool = true) (tree: discrimination_tree) (term: term)
  (expected: term) =
  
  fun () ->
    let renumbered =
      TermDiscriminationTree.renumber_term tree term
    in
    let success =
...
let not_renumber_term (tree: discrimination_tree) (term: term)
  (expected: term) =
  renumber_term ~success_desired:false tree term expected


let tree =
  TermDiscriminationTree.create p_a 0
...
let build_tree (nodes: string list) : term discrimination_tree =
  match nodes with
    | [] ->
	failwith "build_tree"
	...
	(fun tree parse ->
	   let term =
	     Read_darwin.to_term parse
	   in
	     TermDiscriminationTree.insert tree term term Unification.are_terms_variants Term.term_to_string
	)
	(TermDiscriminationTree.create (Read_darwin.to_term head) (Read_darwin.to_term head))
	tail
  ...
  fun () ->
    ...
    in
      print_endline ("insert:\n"
		     ^ Term.term_to_string term
		     ^ "\ninto\n"
		     ^ TermDiscriminationTree.to_string tree
		     ^ "\nis\n"
		     ^ TermDiscriminationTree.to_string inserted)
...
(*** subsumes ***)
let subsumes ?(success_desired:bool = true) (tree: term discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      TermDiscriminationTree.subsumes tree term
    in
    let success =
...
(*** p_unifies ***)
let p_unifies ?(success_desired:bool = true) (tree: discrimination_tree) (parse: string) (expected: string list) =
  fun () ->
    let term =
      Read_darwin.to_term parse
    in
    ...
    in
    let nodes =
      TermDiscriminationTree.p_unifies tree term
    in
    let success =
      Tools.lists_unordered_equal Unification.are_terms_variants nodes expected
    in
...
(*** is_msg ***)
let is_msg ?(success_desired:bool = true) (tree: discrimination_tree) (parse: (string * string)) (expected: string list) =
  fun () ->
    let term1, term2 =
    ...
    in
    let nodes =
      TermDiscriminationTree.is_msg tree term1 term2
    in
    let success =
...
(*** mscg_of_terms ***)
let mscg_of_terms ?(success_desired:bool = true) (tree: discrimination_tree) (parse1: string) (parse2: string)
  (expected: string) =
  fun () ->
    let term1 =
      TermDiscriminationTree.renumber_term tree (Read_darwin.to_term parse1)
    and term2 =
      TermDiscriminationTree.renumber_term tree (Read_darwin.to_term parse2)
    and expected =
      Read_darwin.to_term expected
    in
    let mscg, subst1, subst2 =
      TermDiscriminationTree.mscg_of_terms tree term1 term2 []
    in
    let applied1 =
...
let not_mscg_of_terms (tree: discrimination_tree) (parse1: string) (parse2: string) (expected: string) =
  mscg_of_terms ~success_desired:false tree parse1 parse2 expected


let tree =
  TermDiscriminationTree.create (Read_darwin.to_term "a") 0
...
(*
(*** mscg_of_substs ***)
let mscg_of_substs ?(success_desired:bool = true) (tree: discrimination_tree) (subst1: subst) (subst2: subst) =
  fun () ->
    let mscg, mscg_subst1, mscg_subst2 =
      TermDiscriminationTree.mscg_of_substs tree subst1 subst2 []
    in
      print_endline
	("mscg_of_substs:\n"
	 ^ Subst.subst_to_string subst1 ^ "\n"
...
(*
let not_mscg_of_substs (tree: discrimination_tree) (parse1: string) (parse2: string) (expected: string) =

...
line ::= INCR IDENTIFIER(id).                           { pParser->incr(id.index); }
...
line ::= STATIC signed(id) SLASH NUMBER(num).          { OUT->dynamic(id.index, num.number, false); }
line ::= DYNAMIC signed(id) SLASH NUMBER(num).         { OUT->dynamic(id.index, num.number, true); }
